...
% Display the image that we want to hide/embed.
subplot(3, 3, 1);
imshow(hiddenImage, []);
...
% I could have used a different binary image but I wanted the demo to run
% with just a single image, so I just thresholded the cover image. 
% Feel free to substitute a different binary image if you want.
thresholdValue = 70;
...
% Since it's binary it can be stored in a single bit plane.
...
% Add noise to watermarked image to "attack" it.
...
title(caption, 'FontSize', fontSize);
% Display the watermark image recovered from the noisy/attacked image.
subplot(3, 3, 9);
